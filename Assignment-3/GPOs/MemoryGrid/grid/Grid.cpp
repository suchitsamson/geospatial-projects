...
Grid::Grid (){
    ...
    cout << "MIN_LON = " << MIN_LON << " MAX_LON = " << MAX_LON << " DELTA_LON = " << DELTA_LON << endl;
    cout << "MIN_LAT = " << MIN_LAT << " MAX_LAT = " << MAX_LAT << " DELTA_LAT = " << DELTA_LAT << endl;

    for (int i=0; i<X_granularity; i++){
        for (int j=0; j<Y_granularity; j++){
            Cell* c = new Cell;
            c->setDimensions(MIN_LON+(i*DELTA_LON), MIN_LAT+(j*DELTA_LAT), MIN_LON+((i+1)*DELTA_LON), MIN_LAT+((j+1)*DELTA_LAT));
            //            cout << MIN_LON+(i*DELTA_LON) << " , " <<  MIN_LAT+(j*DELTA_LAT) << " , " <<  MIN_LON+((i+1)*DELTA_LON) << " , " <<  MIN_LAT+((j+1)*DELTA_LAT) << endl;
            c->setType(CELL);
            c->setIndex(i, j);
            table[i][j] = c;
        }
    ...
    while ( !heap->empty() &&  tmp->getMinDist() < best_dist && count < k && v.getResult()->size() < DATASET_SIZE) {
        ...
        if (tmp->getType() == CELL){                               //it is a cell
            ...
            if (direction == UP || direction == DOWN){
                for(double i = (x1+DELTA_LON/2.0); i < x2; i+=DELTA_LON){
                    Cell* cr = getCell(i, (y1+DELTA_LAT/2.0));
                    if(cr != NULL){
                ...
                for(double j = (y1+DELTA_LAT/2); j < y2; j+=DELTA_LAT){
                    Cell* cr = getCell((x1+DELTA_LON/2), j);
                    if(cr != NULL){
                        cr->computeMinDist(x, y);
                        heap->push(cr);
                    }
...
void Grid::getRectangle(int direction, int level, double x, double y, Cell& c){ 
    ...
    int i = (int)((x - MIN_LON)/DELTA_LON);
    int j = (int)((y - MIN_LAT)/DELTA_LAT);

    double x1, y1, x2, y2;

    if (direction == UP){

        x1 = (i - level)*DELTA_LON;
        y1 = (j + 1 + level)*DELTA_LAT;
        x2 = (i + 2 + level)*DELTA_LON;
        y2 = (j + 2 + level)*DELTA_LAT;
        
    }
    else if (direction == DOWN){

        x1 = (i - level-1)*DELTA_LON;
        y1 = (j - 1 - level)*DELTA_LAT;
        x2 = (i + 1 + level)*DELTA_LON;
        y2 = (j - level)*DELTA_LAT;
        
    }
    else if (direction == RIGHT){

        x1 = (i + level + 1)*DELTA_LON;
        y1 = (j - level - 1)*DELTA_LAT;
        x2 = (i + 2 + level)*DELTA_LON;
        y2 = (j + level + 1)*DELTA_LAT;

    }
    else if (direction == LEFT){

        x1 = (i - 1 - level)*DELTA_LON;
        y1 = (j - level)*DELTA_LAT;
        x2 = (i - level)*DELTA_LON;
        y2 = (j + level + 2)*DELTA_LAT;
    ...
    c.setDimensions(x1+MIN_LON, y1+MIN_LAT, x2+MIN_LON, y2+MIN_LAT);
...
list<Cell*>* Grid::getIntersectedCellsWithRectangle(double x1, double y1, double x2, double y2){
    ...
    int x_start = (int)((x1 - MIN_LON)/DELTA_LON);
    int x_end = (int)((x2 - MIN_LON)/DELTA_LON);
    int y_start = (int)((y1 - MIN_LAT)/DELTA_LAT);
    int y_end = (int)((y2 - MIN_LAT)/DELTA_LAT);
    ...
    while ( !heap->empty() &&  tmp->getMinDist() < best_dist && count < k) {
        ...
        if (tmp->getType() == CELL){                               //it is a cell
            ...
            if (direction == UP || direction == DOWN){
                ...
                if (levels[direction] > X_granularity) {
                    delete tmp;
                    tmp = heap->top();
                    continue;
                }
                ...
                if (levels[direction] > Y_granularity) {
                    delete tmp;
                    tmp = heap->top();
                    continue;
                }
    ...
    while ( !heap->empty() &&  tmp->getMinDist() < best_dist && count < k) {
        ...
        if (tmp->getType() == CELL){                               //it is a cell
            ...
            for(it=L->begin(); it != L->end(); ++it){
                ...
                c->setType(POINT);
                c->setID(p->getID());
				c->setIndex(tmp->getIndexI(), tmp->getIndexJ());
                c->setDimensions(p->getX(), p->getY(), p->getX(), p->getY());
                c->computeMinDist(x, y);
                heap->push(c);
            }
...
void Grid::getRange(double x, double y, double radius, Visitor& v){  
	...
	double radius_lat = radius * 360 / EARTH_CIRCUMFERENCE;   // radius_lat is equal no matter what lat or lon is
	double radius_lon = radius * 360 / (EARTH_CIRCUMFERENCE * cos(y * DEG_TO_RAD));    // radius_lon changes according to the value of lat

	// find bottom-left and topright of rectangle which encompasses the range
	int x_start = (int)(((x - radius_lon - BOUNDARY_ERROR) - MIN_LON) / DELTA_LON);
	int x_end = (int)(((x + radius_lon + BOUNDARY_ERROR) - MIN_LON) / DELTA_LON);
	int y_start = (int)(((y - radius_lat - BOUNDARY_ERROR) - MIN_LAT) / DELTA_LAT);
	int y_end = (int)(((y + radius_lat + BOUNDARY_ERROR) - MIN_LAT) / DELTA_LAT);
    ...
    for(int i = x_start; i <= x_end && x_end < X_granularity; i++){
        for(int j = y_start; j <= y_end && y_end < Y_granularity; j++){
            c = getCell(i, j);
            count++;
            visits++;
...
vector<res_point*>* Grid::getRange(double x, double y, double radius){  
    ...
    if(x_end >= X_granularity)
        x_end = X_granularity-1;

    if(y_end >= Y_granularity)
        y_end = Y_granularity-1;
...
set<res_point*, res_point_ascending_id>* Grid::getSetRange(double x, double y, double radius){
    ...
    int x_start = (int)(((x-radius) - MIN_LON)/DELTA_LON);
    int x_end = (int)(((x+radius) - MIN_LON)/DELTA_LON);
    int y_start = (int)(((y-radius) - MIN_LAT)/DELTA_LAT);
    int y_end = (int)(((y+radius) - MIN_LAT)/DELTA_LAT);
...
bool Grid::loadFromFile(const char* fileName){
    ...
    while (fin)
    {
        ...
        if(x1> MIN_LON && x1 < MAX_LON && y1 > MIN_LAT && y1 < MAX_LAT){
...
double Grid::estimateNearestDistance(double x, double y, int k){
    ...
    if(DATASET_SIZE<k)  {
        HP_lastCells[0][0] = X_granularity-1;        HP_lastCells[0][1] = Y_granularity-1;
        HP_lastCells[1][0] =X_granularity-1;        HP_lastCells[1][1] =0;
        HP_lastCells[2][0]= 0;        HP_lastCells[2][1]= 0;
        HP_lastCells[3][0] =0;        HP_lastCells[3][1] =Y_granularity-1;
    ...
    do{
        ...
        for(list<Cell*>::iterator it = intersectedCells->begin(); it!= intersectedCells->end(); ++it){
            ...
            intersectedVolPercentage = tmp->intersectedVolumeWithRectangle(x-L/2, y-L/2, x+L/2, y+L/2)/(DELTA_LON*DELTA_LAT);
        ...
        if(k <= u){
            ...
            if(direction == 0){
                if(HP_lastCells[0][0] > 0 && HP_lastCells[0][0] < X_granularity-1){
                    HP_lastCells[0][0] = HP_lastCells[0][0] +1;
                    flag =true;
                }
            }
            else if(direction == 1){
                if(HP_lastCells[1][0] > 0 && HP_lastCells[1][0] < X_granularity-1){
                    HP_lastCells[1][0] = HP_lastCells[1][0] -1;
                    flag =true;
                }
            }
            else if(direction == 2){
                if(HP_lastCells[2][1] > 0 && HP_lastCells[2][1] < Y_granularity-1){
                    HP_lastCells[2][1] = HP_lastCells[2][1] +1;
                    flag =true;
                }
            }
            else {
                if(HP_lastCells[3][1] > 0 && HP_lastCells[3][1] < Y_granularity-1){
