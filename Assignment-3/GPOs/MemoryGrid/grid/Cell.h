...
public:
    struct ascending : public binary_function<Cell*, Cell*, bool>
    {
        bool operator()(const Cell* __x, const Cell* __y) const { 
			if (__x->c_minDist > __y->c_minDist) return true;
			if (__x->c_minDist < __y->c_minDist) return false;
			if ((__x->cell_type == POINT) ^ (__y->cell_type == POINT))
				return (__x->cell_type == POINT);
			if ((__x->cell_type == POINT) && (__y->cell_type == POINT))
				return (__x->c_id > __y->c_id);
			return false;
		}
    };
